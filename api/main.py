...
def create_job(job_id: str, job_type: str, payload: dict) -> dict:
    return store.enqueue_job(job_id, job_type, payload)
...
def plan_training(body: TrainingPlanRequest) -> dict:
    ...
    job = store.enqueue_job(plan.job_id, plan.job_type, plan.model_dump())
    return {"plan": plan.model_dump(), "job": job}


@app.post("/runtime/models")
def register_runtime_model(body: RuntimeModelRegister) -> dict:
    manifest = ModelManifest(**body.model_dump())
    return runtime_registry.register_model(manifest)
...
def register_runtime_node(body: RuntimeNodeRegister) -> dict:
    ...
    return runtime_registry.register_runtime(profile)
...
def chat_completions(body: ChatCompletionRequest) -> dict:
    ...
    try:
        ...
        source = "fallback"
    return build_chat_completion_response(body, answer, source)
...
def record_usage_event(user_id: str = "local", event_type: str = "manual", quantity: float = 1.0, source: str = "api", metadata: dict | None = None) -> dict:
    return usage_store.record(user_id, event_type, quantity, source, metadata)
